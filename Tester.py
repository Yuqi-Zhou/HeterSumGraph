...
class SLTester(TestPipLine):
    ...
    def evaluation(self, G, index, dataset, blocking=False):
        ...
        # 得出 outputs 后, 和 train.py 里的 run_training 函数一样算loss
        snode_id = G.filter_nodes(lambda nodes: nodes.data["dtype"] == 1)  # 得到所有句子节点的 id
        label = G.ndata["label"][snode_id].sum(-1)    # [n_nodes]  # 得到一个标签列表 [0, 0, 1, 0, ...(一共有句子数个元素, 为1表示该句是文摘)]
        ...
        G.nodes[snode_id].data["p"] = outputs   # 给所有的句子节点赋上 p 特征, 值为 outputs
        glist = dgl.unbatch(G)
        for j in range(len(glist)):
            idx = index[j]
            example = dataset.get_example(idx)  # 得到验证/测试集文件上的一行
            ...
            self._hyps.append(hyps)  # 添加抽取的文摘到列表中
            self._refer.append(refer)   # 添加黄金文摘到列表中
