...
from module.dataloader import ExampleSet, MultiExampleSet, graph_collate_fn, LoadHiExampleSet
                ...
                if _DEBUG_FLAG_:
                    for name, param in model.named_parameters():
                        if param.requires_grad:
                            ...
                            logger.debug('param.grad.data.sum(): ' + str(param.grad.data.sum()))
...
def run_eval(model, loader, valset, hps, best_loss, best_F, non_descent_cnt, saveNo):
    ...
    if best_loss is None or running_avg_loss < best_loss:
        ...
        if best_loss is not None:
            logger.info('[INFO] Found new best model with %.6f running_avg_loss. The original loss is %.6f, Saving to %s',
                float(running_avg_loss), float(best_loss), bestmodel_save_path)
        else:
            logger.info('[INFO] Found new best model with %.6f running_avg_loss. The original loss is None, Saving to %s',
...
def main():
    ...
    parser.add_argument('--dgl_dir', type=str, default='dgl_data/cles', help='The dgl dataset directory')
    ...
    if hps.model == "HSG":
        ...
        valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=hps.batch_size, shuffle=False,
                                                   ...
                                                   num_workers=hps.num_workers)  # Shuffle Must be False for ROUGE evaluation
    else:
