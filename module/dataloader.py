...
class ExampleSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, vocab, doc_max_timesteps, sent_max_len, filter_word_path, w2s_path,
                 use_interest=False, interest_path='', language=''):
        ...
        if use_interest:
            ...
            assert len(self.interest_list) == len(self.example_list)
    ...
    def CreateGraph(self, input_pad, label, w2s_w, a_interest_dict):
        ...
        if self.use_interest:
            ...
            for ii in ['s_text', 'd_text']:
                for a_igedge_list in a_interest_dict[ii]:
                    num_igw = 1 if ii == 's_text' else 0
                    a_igword = a_igedge_list[num_igw].lower()
                    a_igwid = self.vocab.word2id(a_igword)
                    num_w = 0 if ii == 's_text' else 1
                    a_word = a_igedge_list[num_w].lower()
            ...
            G.ndata["unit"][w_nodes + N:] = torch.ones(ignode_num) * 2
            G.ndata["dtype"][w_nodes + N:] = torch.ones(ignode_num) * 2
            G.ndata["igwid"] = torch.LongTensor([0]*ignid)
            G.ndata["igwid"][w_nodes + N:] = torch.LongTensor(list(ignid2wid.values()))

        G.set_e_initializer(dgl.init.zero_initializer)  # 初始化空边
        ...
        if self.use_interest:
            ignode_num = len(ignid2wid)
            igwnodeid_list = [j for j in range(w_nodes+N, w_nodes+N+ignode_num)]
            igwid_list = G.nodes[igwnodeid_list].data["igwid"]
            wnodeid_list = [i for i in range(w_nodes)]
            wid_list = G.nodes[wnodeid_list].data["id"]
            G.edata["ig_weight"] = torch.LongTensor([0]*G.number_of_edges())

            for ii in ['s_text', 'd_text', 'sd_text']:
                for a_igedge_list in a_interest_dict[ii]:
                    word0_id = self.vocab.word2id(a_igedge_list[0])
                    word1_id = self.vocab.word2id(a_igedge_list[1])
                    ig_weight_box = np.round(a_igedge_list[2]*9)
                    if ii == 's_text':
                        if word0_id in wid_list and word1_id in igwid_list:
                            G.add_edge(wid2nid[word0_id], igwid2nid[word1_id], data={"ig_weight": torch.LongTensor([ig_weight_box]), "dtype": torch.Tensor([2])})
                            G.add_edge(igwid2nid[word1_id], wid2nid[word0_id], data={"ig_weight": torch.LongTensor([ig_weight_box]), "dtype": torch.Tensor([2])})
                    elif ii == 'd_text':
                        if word0_id in igwid_list and word1_id in wid_list:
                            G.add_edge(igwid2nid[word0_id], wid2nid[word1_id], data={"ig_weight": torch.LongTensor([ig_weight_box]), "dtype": torch.Tensor([3])})
                            G.add_edge(wid2nid[word1_id], igwid2nid[word0_id], data={"ig_weight": torch.LongTensor([ig_weight_box]), "dtype": torch.Tensor([3])})
                    elif ii == 'sd_text':
                        if word0_id in wid_list and word1_id in wid_list:
                            G.add_edge(wid2nid[word0_id], wid2nid[word1_id], data={"ig_weight": torch.LongTensor([ig_weight_box]), "dtype": torch.Tensor([4])})
                            G.add_edge(wid2nid[word1_id], wid2nid[word0_id], data={"ig_weight": torch.LongTensor([ig_weight_box]), "dtype": torch.Tensor([4])})
    ...
    def __getitem__(self, index):
        ...
        if self.use_interest:
            a_interest_dict = self.interest_list[index]
            G = self.CreateGraph(input_pad, label, w2s_w, a_interest_dict)
        else:
...
def readJson(fname):  # 用来读取数据集和图数据中的 jsonl 文件, 返回一个列表, 一个元素表示一行(一个json). 文件中的斜杠加u必须是双斜杠, 读取到内存中才会是单斜杠
    data = []
